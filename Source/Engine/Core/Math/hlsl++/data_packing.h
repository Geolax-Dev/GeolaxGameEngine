...
#include <Core/Math/hlsl++/half.h>
#include <Core/Math/hlsl++/dependent.h>

GGE_module_export namespace GGE
{
	GGE_inline uint32_t pack_float4_rgba8_unorm(const float4& v)
	{
		return _GGE_pack_epu32_rgba8_unorm(v.vec);
	}
	
	GGE_inline float4 unpack_rgba8_unorm_float4(uint32_t p)
	{
		return float4(_GGE_unpack_rgba8_unorm_epu32(p));
	}
	
	GGE_inline uint32_t pack_float4_rgba8_snorm(const float4& v)
	{
		return _GGE_pack_epu32_rgba8_snorm(v.vec);
	}

	GGE_inline float4 unpack_rgba8_snorm_float4(uint32_t p)
	{
		return float4(_GGE_unpack_rgba8_snorm_epu32(p));
	}
	...
	GGE_inline uint32_t pack_float3_rgb9e5f(const float3& v)
	{
	...
	GGE_inline float3 unpack_rgb9e5f_float3(uint32_t p)
	{
