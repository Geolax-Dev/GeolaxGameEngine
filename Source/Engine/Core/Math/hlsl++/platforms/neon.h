...
#if defined(GGE_ARM64)

typedef float64x2_t n128d;

#endif

GGE_inline float32x4_t vmov4q_n_f32(const float x, const float y, const float z, const float w)
{
	const float values[4] = { x, y, z, w };
	return vld1q_f32(values);
}

GGE_inline int32x4_t vmov4q_n_s32(const int32_t x, const int32_t y, const int32_t z, const int32_t w)
{
	const int32_t values[4] = { x, y, z, w };
	return vld1q_s32(values);
}

GGE_inline uint32x4_t vmov4q_n_u32(const uint32_t x, const uint32_t y, const uint32_t z, const uint32_t w)
{
...
GGE_inline float32x4_t vpermq_f32(const float32x4_t x, const uint32_t X, const uint32_t Y, const uint32_t Z, const uint32_t W)
{
	uint8x8x2_t tbl = { { vget_low_f32(x), vget_high_f32(x) } }; // Get floats

	const uint8x8_t idxL = vcreate_u8(((uint64_t)GGE::ControlMask[X]) | (((uint64_t)GGE::ControlMask[Y]) << 32));
	const uint8x8_t rL = vtbl2_u8(tbl, idxL);

	const uint8x8_t idxH = vcreate_u8(((uint64_t)GGE::ControlMask[Z]) | (((uint64_t)GGE::ControlMask[W]) << 32));
	const uint8x8_t rH = vtbl2_u8(tbl, idxH);

	return vcombine_f32(rL, rH);
}

GGE_inline float32x4_t vshufq_f32(const float32x4_t x, const float32x4_t y, const uint32_t X0, const uint32_t Y0, const uint32_t X1, const uint32_t Y1)
{
	uint8x8x4_t tbl = { { vget_low_f32(x), vget_high_f32(x), vget_low_f32(y), vget_high_f32(y) } };

	const uint8x8_t idxL = vcreate_u8(((uint64_t)GGE::ControlMaskX[X0]) | (((uint64_t)GGE::ControlMaskX[Y0]) << 32));
	const uint8x8_t rL = vtbl4_u8(tbl, idxL);

	const uint8x8_t idxH = vcreate_u8(((uint64_t)GGE::ControlMaskY[X1]) | (((uint64_t)GGE::ControlMaskY[Y1]) << 32));
...
// Source http://stackoverflow.com/questions/32536265/how-to-convert-mm-shuffle-ps-sse-intrinsic-to-neon-intrinsic
GGE_inline int32x4_t vpermq_s32(const int32x4_t x, const uint32_t X, const uint32_t Y, const uint32_t Z, const uint32_t W)
{
...
GGE_inline int32x4_t vshufq_s32(const int32x4_t x, const int32x4_t y, const uint32_t X0, const uint32_t Y0, const uint32_t X1, const uint32_t Y1)
{
...
// Source http://stackoverflow.com/questions/32536265/how-to-convert-mm-shuffle-ps-sse-intrinsic-to-neon-intrinsic
GGE_inline uint32x4_t vpermq_u32(const uint32x4_t x, const uint32_t X, const uint32_t Y, const uint32_t Z, const uint32_t W)
{
...
GGE_inline uint32x4_t vshufq_u32(const uint32x4_t x, const int32x4_t y, const uint32_t X0, const uint32_t Y0, const uint32_t X1, const uint32_t Y1)
{
...
GGE_inline float32x4_t vrsqrtq_f32(float32x4_t x)
{
...
GGE_inline float32x4_t vrndmq_f32(float32x4_t x)
{
...
GGE_inline float32x4_t vrndpq_f32(float32x4_t x)
{
	float32x4_t ones = vmovq_n_f32(1.0f);
	float32x4_t trnc = vcvtq_f32_s32(vcvtq_s32_f32(x)); // Truncate
...
GGE_inline float32x4_t vrndnq_f32(float32x4_t x)
{
...
GGE_inline float32x4_t vsqrtq_f32(float32x4_t x)
{
...
GGE_inline float32x4_t vdivq_f32(float32x4_t x, float32x4_t y)
{
...
GGE_inline uint32x4_t vdivq_u32(uint32x4_t x, uint32x4_t y)
{
	return vcvtq_u32_f32(vdivq_f32(vcvtq_f32_u32(x), vcvtq_f32_u32(y)));
}

GGE_inline float32x4_t vrcpq_f32(float32x4_t x)
{
...
#define _GGE_set1_ps(x)						vmovq_n_f32((x))
#define _GGE_set_ps(x, y, z, w)				vmov4q_n_f32((x), (y), (z), (w))
#define _GGE_setzero_ps()					vmovq_n_f32(0.0f)

#define _GGE_add_ps(x, y)					vaddq_f32((x), (y))
#define _GGE_sub_ps(x, y)					vsubq_f32((x), (y))
#define _GGE_mul_ps(x, y)					vmulq_f32((x), (y))
#define _GGE_div_ps(x, y)					vdivq_f32(x, y)

#define _GGE_rcp_ps(x)						vrcpq_f32((x))

#define _GGE_neg_ps(x)						veorq_u32(vreinterpretq_u32_f32((x)), vmovq_n_u32(0x80000000u))

#define _GGE_madd_ps(x, y, z)				vmlaq_f32((z), (x), (y)) // x * y + z but vmlaq_f32 does y * z + x
#define _GGE_msub_ps(x, y, z)				_GGE_neg_ps(vmlsq_f32((z), (x), (y))) // x * y - z but vmlsq_f32 does z - x * y
#define _GGE_subm_ps(x, y, z)				vmlsq_f32((x), (y), (z)) // x - y * z

#define _GGE_abs_ps(x)						vabsq_f32((x))

#define _GGE_sqrt_ps(x)						vsqrtq_f32((x))
#define _GGE_rsqrt_ps(x)						vrsqrtq_f32((x))

#define _GGE_cmpeq_ps(x, y)					vceqq_f32((x), (y))
#define _GGE_cmpneq_ps(x, y)					veorq_u32(vreinterpretq_u32_f32(vceqq_f32((x), (y))), vmovq_n_u32(0xffffffffu))

#define _GGE_cmpgt_ps(x, y)					vcgtq_f32((x), (y))
#define _GGE_cmpge_ps(x, y)					vcgeq_f32((x), (y))

#define _GGE_cmplt_ps(x, y)					vcltq_f32((x), (y))
#define _GGE_cmple_ps(x, y)					vcleq_f32((x), (y))

#define _GGE_max_ps(x, y)					vmaxq_f32((x), (y))
#define _GGE_min_ps(x, y)					vminq_f32((x), (y))

#define _GGE_sel_ps(x, y, mask)				vbslq_f32((mask), (y), (x))

// We decompose the mask and turn it into 4 floats
// The input mask follows the format for SSE
#define _GGE_blend_ps(x, y, mask)			vbslq_f32(vmov4q_n_u32(((mask) & 1) * 0xffffffff, (((mask) >> 1) & 1) * 0xffffffff, (((mask) >> 2) & 1) * 0xffffffff, (((mask) >> 3) & 1) * 0xffffffff), (y), (x))

#define _GGE_trunc_ps(x)						vcvtq_f32_s32(vcvtq_s32_f32(x))
#define _GGE_floor_ps(x)						vrndmq_f32((x))
#define _GGE_ceil_ps(x)						vrndpq_f32((x))
#define _GGE_round_ps(x)						vrndnq_f32((x))

#define _GGE_frac_ps(x)						vsubq_f32((x), vrndmq_f32(x))

#define _GGE_clamp_ps(x, minx, maxx)			vmaxq_f32(vminq_f32((x), (maxx)), (minx))
#define _GGE_sat_ps(x)						vmaxq_f32(vminq_f32((x), f4_1), f4_0)

// http://codesuppository.blogspot.co.uk/2015/02/sse2neonh-porting-guide-and-header-file.html
#define _GGE_and_ps(x, y)					vandq_f32((x), (y))
#define _GGE_andnot_ps(x, y)					vbicq_f32((y), (x))
#define _GGE_not_ps(x)						vmvnq_f32((x))
#define _GGE_or_ps(x, y)						vorrq_f32((x), (y))
#define _GGE_xor_ps(x, y)					veorq_f32((x), (y))
...
#define _GGE_movelh_ps(x, y)					vcombine_f32(vget_low_f32(x), vget_low_f32(y))
...
#define _GGE_movehl_ps(x, y)					vcombine_f32(vget_high_f32(y), vget_high_f32(x))

// SSE: Duplicate odd-indexed single-precision (32-bit) floating-point elements from a, and store the results in dst.
#define _GGE_movehdup_ps(x)					vcombine_f32(vdup_lane_f32(vget_low_f32(x), 1), vdup_lane_f32(vget_high_f32(x), 1))

#define _GGE_perm_ps(x, X, Y, Z, W)			vpermq_f32((x), X, Y, Z, W)

#define _GGE_shuffle_ps(x, y, X, Y, A, B)	vshufq_f32((x), (y), X, Y, A, B)

#define _GGE_unpacklo_ps(x, y)				vzipq_f32(x, y).val[0]
#define _GGE_unpackhi_ps(x, y)				vzipq_f32(x, y).val[1]
...
#define _GGE_any1_ps(x)						_GGE_any1_epi32(vreinterpretq_u32_f32(x))
#define _GGE_any2_ps(x)						_GGE_any2_epi32(vreinterpretq_u32_f32(x))
#define _GGE_any3_ps(x)						_GGE_any3_epi32(vreinterpretq_u32_f32(x))
#define _GGE_any4_ps(x)						_GGE_any4_epi32(vreinterpretq_u32_f32(x))

#define _GGE_all1_ps(x)						_GGE_all1_epi32(vreinterpretq_u32_f32(x))
#define _GGE_all2_ps(x)						_GGE_all2_epi32(vreinterpretq_u32_f32(x))
#define _GGE_all3_ps(x)						_GGE_all3_epi32(vreinterpretq_u32_f32(x))
#define _GGE_all4_ps(x)						_GGE_all4_epi32(vreinterpretq_u32_f32(x))
...
GGE_inline void _GGE_store4x4_ps(float* dst, n128 src0, n128 src1, n128 src2, n128 src3)
{
	vst1q_f32(dst, src0);
...
#define _GGE_set1_epi32(x)					vmovq_n_s32((x))
#define _GGE_set_epi32(x, y, z, w)			vmov4q_n_s32((x), (y), (z), (w))
#define _GGE_setzero_epi32()					vmovq_n_s32(0)

#define _GGE_add_epi32(x, y)					vaddq_s32((x), (y))
#define _GGE_sub_epi32(x, y)					vsubq_s32((x), (y))
#define _GGE_mul_epi32(x, y)					vmulq_s32((x), (y))
#define _GGE_div_epi32(x, y)					vdivq_s32((x), (y))

#define _GGE_neg_epi32(x)					vaddq_s32(veorq_u32(vreinterpretq_u32_s32(x), vmovq_n_u32(0xffffffffu)), vmovq_n_u32(1))

#define _GGE_madd_epi32(x, y, z)				vmlaq_s32((z), (x), (y))
#define _GGE_msub_epi32(x, y, z)				_GGE_neg_epi32(vmlsq_s32((z), (x), (y))) // Negate because vmlsq_u32 does z - x * y and we want x * y - z
#define _GGE_subm_epi32(x, y, z)				vmlsq_s32((z), (x), (y))

#define _GGE_abs_epi32(x)					vabsq_s32((x))

#define _GGE_cmpeq_epi32(x, y)				vceqq_s32((x), (y))
#define _GGE_cmpneq_epi32(x, y)				veorq_s32(vceqq_s32((x), (y)), vmovq_n_u32(0xffffffffu))

#define _GGE_cmpgt_epi32(x, y)				vcgtq_s32((x), (y))
#define _GGE_cmpge_epi32(x, y)				vcgeq_s32((x), (y))

#define _GGE_cmplt_epi32(x, y)				vcltq_s32((x), (y))
#define _GGE_cmple_epi32(x, y)				vcleq_s32((x), (y))

#define _GGE_max_epi32(x, y)					vmaxq_s32((x), (y))
#define _GGE_min_epi32(x, y)					vminq_s32((x), (y))

#define _GGE_sel_epi32(x, y, mask)			vbslq_s32((mask), (y), (x))
#define _GGE_blend_epi32(x, y, mask)			vbslq_s32(vmov4q_n_u32(~(((mask) & 1) * 0xffffffff), ~((((mask) >> 1) & 1) * 0xffffffff), ~((((mask) >> 2) & 1) * 0xffffffff), ~((((mask) >> 3) & 1) * 0xffffffff)), (x), (y))

#define _GGE_clamp_epi32(x, minx, maxx)		vmaxq_s32(vminq_s32((x), (maxx)), (minx))
#define _GGE_sat_epi32(x)					vmaxq_s32(vminq_s32((x), i4_1), i4_0)

#define _GGE_and_si128(x, y)					vandq_s32((x), (y))
#define _GGE_andnot_si128(x, y)				vandq_s32((y), (x))
#define _GGE_not_si128(x)					vmvnq_s32((x))
#define _GGE_or_si128(x, y)					vorrq_s32((x), (y))
#define _GGE_xor_si128(x, y)					veorq_s32((x), (y))

#define _GGE_perm_epi32(x, X, Y, Z, W)		vpermq_s32((x), X, Y, Z, W)
#define _GGE_shuffle_epi32(x, y, X, Y, A, B)	vshufq_s32((x), (y), X, Y, A, B)

#define _GGE_castps_si128(x)					vreinterpretq_s32_f32((x))
#define _GGE_castsi128_ps(x)					vreinterpretq_f32_s32((x))

#define _GGE_cvttps_epi32(x)					vcvtq_s32_f32((x))
#define _GGE_cvtepi32_ps(x)					vcvtq_f32_s32((x))

#define _GGE_slli_epi32(x, y)				vshlq_n_s32((x), (y))
#define _GGE_srli_epi32(x, y)				vshrq_n_s32((x), (y))

// From the documentation of vshlq_s32: Vector shift left: Vr[i] := Va[i] << Vb[i] (negative values shift right)
#define _GGE_sllv_epi32(x, y)				vshlq_s32((x), (y))
#define _GGE_srlv_epi32(x, y)				vshlq_s32((x), _GGE_neg_epi32(y))
...
#define _GGE_set1_epu32(x)					vmovq_n_u32((x))
#define _GGE_set_epu32(x, y, z, w)			vmov4q_n_u32((x), (y), (z), (w))
#define _GGE_setzero_epu32()					vmovq_n_u32(0)

#define _GGE_add_epu32(x, y)					vaddq_u32((x), (y))
#define _GGE_sub_epu32(x, y)					vsubq_u32((x), (y))
#define _GGE_mul_epu32(x, y)					vmulq_u32((x), (y))
#define _GGE_div_epu32(x, y)					vdivq_u32((x), (y))

#define _GGE_madd_epu32(x, y, z)				vmlaq_u32((z), (x), (y))
#define _GGE_msub_epu32(x, y, z)				_GGE_neg_epi32(vmlsq_u32((z), (x), (y))) // Negate because vmlsq_u32 does z - x * y and we want x * y - z
#define _GGE_subm_epu32(x, y, z)				vmlsq_u32((z), (x), (y))

#define _GGE_cmpeq_epu32(x, y)				vceqq_u32((x), (y))
#define _GGE_cmpneq_epu32(x, y)				veorq_u32(vceqq_u32((x), (y)), vmovq_n_u32(0xffffffffu))

#define _GGE_cmpgt_epu32(x, y)				vcgtq_u32((x), (y))
#define _GGE_cmpge_epu32(x, y)				vcgeq_u32((x), (y))

#define _GGE_cmplt_epu32(x, y)				vcltq_u32((x), (y))
#define _GGE_cmple_epu32(x, y)				vcleq_u32((x), (y))

#define _GGE_max_epu32(x, y)					vmaxq_u32((x), (y))
#define _GGE_min_epu32(x, y)					vminq_u32((x), (y))

#define _GGE_sel_epu32(x, y, mask)			vbslq_u32((mask), (y), (x))
#define _GGE_blend_epu32(x, y, mask)			vbslq_u32(vmov4q_n_u32(~(((mask) & 1) * 0xffffffff), ~((((mask) >> 1) & 1) * 0xffffffff), ~((((mask) >> 2) & 1) * 0xffffffff), ~((((mask) >> 3) & 1) * 0xffffffff)), (x), (y))

#define _GGE_clamp_epu32(x, minx, maxx)		vmaxq_u32(vminq_u32((x), (maxx)), (minx))
#define _GGE_sat_epu32(x)					vmaxq_u32(vminq_u32((x), i4_1), i4_0)

#define _GGE_perm_epu32(x, X, Y, Z, W)		vpermq_u32((x), X, Y, Z, W)
#define _GGE_shuffle_epu32(x, y, X, Y, A, B)	vshufq_u32((x), (y), X, Y, A, B)

#define _GGE_cvttps_epu32(x)					vcvtq_u32_f32((x))
#define _GGE_cvtepu32_ps(x)					vcvtq_f32_u32((x))

#define _GGE_slli_epu32(x, y)				vshlq_n_u32((x), (y))
#define _GGE_srli_epu32(x, y)				vshrq_n_u32((x), (y))

// From the documentation of vshlq_s32: Vector shift left: Vr[i] := Va[i] << Vb[i] (negative values shift right)
#define _GGE_sllv_epu32(x, y)				vshlq_u32((x), (y))
#define _GGE_srlv_epu32(x, y)				vshlq_u32((x), _GGE_neg_epi32(y))
...
GGE_inline void _GGE_store1_epu32(uint32_t* dst, n128u src)
{
	vst1q_lane_u32(dst, src, 0);
}

GGE_inline void _GGE_store2_epu32(uint32_t* dst, n128u src)
{
	vst1_u32(dst, vget_low_u32(src));
}

GGE_inline void _GGE_store3_epu32(uint32_t* dst, n128u src)
{
	vst1_u32(dst, vget_low_u32(src));
	vst1q_lane_u32(dst + 2, src, 2);
}

GGE_inline void _GGE_store4_epu32(uint32_t* dst, n128u src)
{
	vst1q_u32(dst, src);
}

GGE_inline void _GGE_load1_epu32(n128u& dst, const uint32_t* src)
{
	dst = vld1q_lane_u32(src, dst, 0);
}

GGE_inline void _GGE_load2_epu32(n128u& dst, const uint32_t* src)
{
	int32x2_t t = vld1_u32(src); // Load the two values
	dst = vcombine_u32(t, t); // Replicate in the other two to create a float32x4_t
}

GGE_inline void _GGE_load3_epu32(n128u& dst, const uint32_t* src)
{
	int32x2_t t = vld1_u32(src); // Load the two values
	dst = vcombine_u32(t, t); // Replicate in the other two to create a float32x4_t
...
GGE_inline void _GGE_load4_epu32(n128u& dst, const uint32_t* src)
{
	dst = vld1q_u32(src);
}
...
GGE_inline float64x2_t vmov2q_n_f64(const double x, const double y)
{
	const double values[2] = { x, y };
	return vld1q_f64(values);
}

GGE_inline int64x2_t vmov2q_n_s64(const int64_t x, const int64_t y)
{
	const int64_t values[2] = { x, y };
	return vld1q_s64(values);
}

GGE_inline uint64x2_t vmov2q_n_u64(const uint64_t x, const uint64_t y)
{
...
GGE_inline float64x2_t permute(const float64x2_t x);

template<> GGE_inline float64x2_t permute<0, 0>(const float64x2_t x) { return vcombine_f64(vget_low_f64(x), vget_low_f64(x)); }

template<> GGE_inline float64x2_t permute<0, 1>(const float64x2_t x) { return x; }

template<> GGE_inline float64x2_t permute<1, 0>(const float64x2_t x) { return vcombine_f64(vget_high_f64(x), vget_low_f64(x)); }

template<> GGE_inline float64x2_t permute<1, 1>(const float64x2_t x) { return vcombine_f64(vget_high_f64(x), vget_high_f64(x)); }

#define vpermq_f64(x, X, Y) permute<X, Y>(x)

template<int X, int Y>
GGE_inline float64x2_t shuffle(const float64x2_t x, const float64x2_t y);

template<> GGE_inline float64x2_t shuffle<0, 0>(const float64x2_t x, const float64x2_t y) { return vcombine_f64(vget_low_f64(x), vget_low_f64(y)); }

template<> GGE_inline float64x2_t shuffle<0, 1>(const float64x2_t x, const float64x2_t y) { return vcombine_f64(vget_low_f64(x), vget_high_f64(y)); }

template<> GGE_inline float64x2_t shuffle<1, 0>(const float64x2_t x, const float64x2_t y) { return vcombine_f64(vget_high_f64(x), vget_low_f64(y)); }

template<> GGE_inline float64x2_t shuffle<1, 1>(const float64x2_t x, const float64x2_t y) { return vcombine_f64(vget_high_f64(x), vget_high_f64(y)); }

#define vshufq_f64(x, y, X, Y) shuffle<X, Y>(x, y)

GGE_inline float64x2_t vrcpq_f64(float64x2_t x)
{
...
GGE_inline float64x2_t vrsqrtq_f64(float64x2_t x)
{
...
#define _GGE_set1_pd(x)						vmovq_n_f64(x)
#define _GGE_set_pd(x, y)					vmov2q_n_f64((x), (y))
#define _GGE_setzero_pd()					vmovq_n_f64(0.0)

#define _GGE_add_pd(x, y)					vaddq_f64((x), (y))
#define _GGE_sub_pd(x, y)					vsubq_f64((x), (y))
#define _GGE_mul_pd(x, y)					vmulq_f64((x), (y))
#define _GGE_div_pd(x, y)					vdivq_f64((x), (y))

#define _GGE_rcp_pd(x)						vrcpq_f64(x)

#define _GGE_neg_pd(x)						veorq_u64(vreinterpretq_u32_f32(x), d2negativeMask)

#define _GGE_madd_pd(x, y, z)				vmlaq_f64((z), (x), (y))
#define _GGE_msub_pd(x, y, z)				_GGE_neg_pd(vmlsq_f64((z), (x), (y))) // Negate because vmlsq_f32 does z - x * y and we want x * y - z
#define _GGE_subm_pd(x, y, z)				vmlsq_f64((z), (x), (y))

#define _GGE_abs_pd(x)						vabsq_f64(x)

#define _GGE_sqrt_pd(x)						vsqrtq_f64(x)
#define _GGE_rsqrt_pd(x)						vrsqrtq_f64(x)

#define _GGE_cmpeq_pd(x, y)					vceqq_f64((x), (y))
#define _GGE_cmpneq_pd(x, y)					veorq_u64(vreinterpretq_u64_f64(vceqq_f64((x), (y))), vmovq_n_u64(0xffffffffffffffffu))

#define _GGE_cmpgt_pd(x, y)					vcgtq_f64((x), (y))
#define _GGE_cmpge_pd(x, y)					vcgeq_f64((x), (y))

#define _GGE_cmplt_pd(x, y)					vcltq_f64((x), (y))
#define _GGE_cmple_pd(x, y)					vcleq_f64((x), (y))

#define _GGE_max_pd(x, y)					vmaxq_f64((x), (y))
#define _GGE_min_pd(x, y)					vminq_f64((x), (y))

#define _GGE_trunc_pd(x)						vcvtq_f64_s64(vcvtq_s64_f64(x))
#define _GGE_floor_pd(x)						vrndmq_f64((x))
#define _GGE_ceil_pd(x)						vrndpq_f64((x))
#define _GGE_round_pd(x)						vrndq_f64((x))

#define _GGE_frac_pd(x)						vsubq_f64((x), vrndmq_f64(x))

#define _GGE_clamp_pd(x, minx, maxx)			vmaxq_f64(vminq_f64((x), (maxx)), (minx))
#define _GGE_sat_pd(x)						vmaxq_f64(vminq_f64((x), f4_1), f4_0)

#define _GGE_and_pd(x, y)					vreinterpretq_f64_u64(vandq_u64(vreinterpretq_u64_f64((x)), vreinterpretq_u64_f64((y))))
#define _GGE_andnot_pd(x, y)					vreinterpretq_f64_u64(vbicq_u64(vreinterpretq_u64_f64((y)), vreinterpretq_u64_f64((x))))
#define _GGE_not_pd(x, y)					vreinterpretq_f64_u64(vmvnq_u64(vreinterpretq_u64_f64((y)), vreinterpretq_u64_f64((x))))
#define _GGE_or_pd(x, y)						vreinterpretq_f64_u64(vorrq_u64(vreinterpretq_u64_f64((x)), vreinterpretq_u64_f64((y))))
#define _GGE_xor_pd(x, y)					vreinterpretq_f64_u64(veorq_u64(vreinterpretq_u64_f64((x)), vreinterpretq_u64_f64((y))))

#define _GGE_perm_pd(x, mask)				vpermq_f64((x), (mask) & 1, ((mask) >> 1) & 1)
#define _GGE_shuffle_pd(x, y, mask)			vshufq_f64((x), (y), (mask) & 1, ((mask) >> 1) & 1)

// Bit-select x and y based on the contents of the mask
#define _GGE_sel_pd(x, y, mask)				vbslq_f64((mask), (y), (x))

#define _GGE_blend_pd(x, y, mask)			vbslq_f64(vmov2q_n_u64(((mask) & 1) * 0xffffffffffffffff, (((mask) >> 1) & 1) * 0xffffffffffffffff), (y), (x))
...
GGE_inline uint32_t _GGE_pack_epu32_rgba8_unorm(float32x4_t v)
{
...
GGE_inline float32x4_t _GGE_unpack_rgba8_unorm_epu32(uint32_t p)
{
...
GGE_inline uint32_t _GGE_pack_epu32_rgba8_snorm(float32x4_t v)
{
...
GGE_inline float32x4_t _GGE_unpack_rgba8_snorm_epu32(uint32_t p)
{
